...
-- This script provides immediate performance improvements for the complex product query

-- Step 1: Create optimized indexes (run these first for immediate performance gains)
...
-- Step 6: Create specific views for common use cases
CREATE OR ALTER VIEW vmCore_Product_Active AS
SELECT * FROM vmCore_Product_Optimized WHERE iStatus = 1;

CREATE OR ALTER VIEW vmCore_Product_ByCategory AS
SELECT * FROM vmCore_Product_Optimized WHERE iCategory IS NOT NULL;

-- Step 7: Performance comparison queries
-- Before optimization (original query):
-- SELECT COUNT(*) FROM (
--     -- Your original query here
-- ) as OriginalQuery;

-- After optimization:
-- SELECT COUNT(*) FROM vmCore_Product_Optimized;
