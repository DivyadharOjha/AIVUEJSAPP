<template>
  <div class="h-100 d-flex flex-column">
    ...
    <div
      ...
      v-if="paymentStore.componentRecords.creditCard.length > 0"
    >
      <div class="col-12">
...
const creditCardRecords = computed(() => {
  return paymentStore.componentRecords.creditCard.filter(isCreditCardRecord)
})
...
const processPayment = () => {
  ...
  paymentStore.componentRecords.creditCard.push(paymentRecord)
...
onMounted(() => {
  ...
  if (existingRecords.length > 0) {
    // Filter and validate only credit card records
    const validCreditCardRecords = existingRecords.filter(isCreditCardRecord).map((record) => ({
