...
describe('posBillSettlementCreditCard', () => {
  ...
  it('renders initial UI correctly', () => {
    ...
    // Use HTMLInputElement for type safety and clarity
    const amountInput = wrapper.find('#amount')
    expect((amountInput.element as HTMLInputElement).value).toBe('125.5')
  })

  it('enables pay button when card type is selected and amount is entered', async () => {
    const wrapper = mount(posBillSettlementCreditCard)

    const visaRadio = wrapper.find('input#visa')
  ...
  it('shows table when payment records exist', async () => {
    ...
    store.componentRecords.creditCard.push({
      id: 'test-1',
      type: 'creditCard',
      cardType: 'visa',
      amount: 50,
  ...
  it('filters out records with empty card type', async () => {
    ...
    // Add invalid record with empty card type
    store.componentRecords.creditCard.push({
      id: 'test-2',
      type: 'creditCard',
      cardType: '',
